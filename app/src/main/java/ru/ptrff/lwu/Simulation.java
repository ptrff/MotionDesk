...
public class Simulation {
    ...
    public DelayedRemovalArray<Particle> particles = new DelayedRemovalArray<Particle>();
    ...
    public void render(ShapeRenderer batch, Sprite circle) {
        ...
        for (int i = 0; i < particles.size; i++) {
            ...
            batch.circle(p.pos.x * Gdx.graphics.getWidth()/2.2f, p.pos.y * Gdx.graphics.getHeight()/5.5f, 10);
    ...
    public void spawnParticles(int x, int y) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int i2 = 0; i2 < 4; i2++) {
                num++;
                particles.add(new Particle(this, (x + i * 10 - 100) / 400f, ((Gdx.graphics.getHeight() - y + i2 * 10) - 100) / 400f, a1, a2, false, mass, num));
            }
        }
    }
}